...
The default generation model is `qwen2.5:7b`. A Hugging Face Qwen tokenizer is
...
5. Structured extraction (Qwen2.5 through Ollama)
        |-- Tables 1, 2, 3, 4, and 6: LLM + Pydantic schema
        ...
        `-- Table 7: count distinct papers from verified Table 6 records
                                |
                                `-- data/tables/*.json
```
...
Tables 1, 2, 3, 4, and 6 use the same structured extractor with a different
...
- Missing required source values reject the complete record. Required examples
  include raw material, nutrition parameter/value, and taxonomy name.
...
- Ethnic groups, geographic fields, and taxonomy names must also be connected
  to the target food in the same sentence, table row, or heading/paragraph pair.
...
| Table 7 | Microbe paper frequency | `microbe`, `count` | Derived from verified Table 6; no LLM call |
...
Table 7 counts the number of distinct papers that report each verified Table 6
microbe. Repeated mentions, chunks, pages, and foods within one paper count only
once. The independent verifier regenerates Table 7 from the verified Table 6
records, so its counts are never generated by the language model.
...
- Qwen2.5 7B available in Ollama
...
| `OLLAMA_MODEL` | `qwen2.5:7b` | Ollama generation model |
