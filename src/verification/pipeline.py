...
STANDARD_TABLES = ("table1", "table2", "table3", "table4", "table6")
...
class PostExtractionVerificationPipeline:
    ...
    def _verify_standard_record(
        ...
        verified = verifier.verify(
            candidate,
            cited_chunks,
            target_food=record.get("food_name"),
        )
        if verified is None:
    ...
    def run(self):
        ...
        verified_tables["table7"] = build_microbe_paper_frequency(
            verified_tables["table6"]
        )
        rejected_tables["table7"] = []
        ...
        for table in ALL_TABLES:
            ...
            if table == "table7":
                statuses = Counter(
                    {"derived_from_verified_table6": len(accepted)}
                )
            else:
                statuses = Counter(
                    record.get("evidence_verification", {}).get(
                        "status", "unknown"
                    )
