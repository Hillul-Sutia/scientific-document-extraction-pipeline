...
from src.extraction.aggregation import build_microbe_paper_frequency
from src.extraction.entity_validation import is_plausible_food_name
...
class InvalidJsonThenSucceedLLM(TimeoutThenSucceedLLM):
    def generate(self, prompt: str, json_schema=None, max_output_tokens=512):
        del json_schema, max_output_tokens
        self.prompts.append(prompt)
        if len(self.prompts) == 1:
            return '[{"category": "Fermented Vegetable"'
        chunk_id = re.search(r"chunk_id=([^\s]+)", prompt).group(1)
        return json.dumps([{
            "category": "Fermented Vegetable",
            "type": "Solid Fermented Food",
...
class RetrieverTests(unittest.TestCase):
    ...
    def test_retriever_skips_table_without_table_specific_signal(self):
        chunks = [make_chunk(
            "doc",
            "doc.pdf",
            "c1",
            "Kimchi was included in this review.",
        )]

        selected = EvidenceRetriever(max_chunks=3).retrieve(
            "kimchi",
            chunks,
            "table4",
            seed_chunk_ids=["c1"],
...
class EvidenceVerifierTests(unittest.TestCase):
    ...
    def test_geography_must_be_locally_related_to_food(self):
        chunks = [make_chunk(
            "doc",
            "doc.pdf",
            "c1",
            "Kimchi was studied. The corresponding author is based in Nagaland.",
        )]
        record = {
            "state": "Nagaland",
            "district": None,
        ...
        verified = EvidenceVerifier("table3").verify(
            record,
            chunks,
            target_food="kimchi",
        )
    ...
    def test_microbe_must_be_valid_and_related_to_food(self):
        chunks = [make_chunk(
            "doc",
            "doc.pdf",
            "c1",
            "Lactobacillus plantarum was isolated from Kimchi.",
        )]
        valid_record = {
            "taxonomy_level": "species",
            "taxonomy_name": "Lactobacillus plantarum",
            "evidence_chunk_ids": ["c1"],
        }
        invalid_record = {
            "taxonomy_level": "genus",
            "taxonomy_name": "Kimchi",
            "evidence_chunk_ids": ["c1"],
        }

        self.assertIsNotNone(EvidenceVerifier("table6").verify(
            valid_record,
            chunks,
            target_food="kimchi",
        ))
        self.assertIsNone(EvidenceVerifier("table6").verify(
            invalid_record,
            chunks,
            target_food="kimchi",
        ))

    def test_mojibake_ethnic_group_is_normalized_after_verification(self):
        chunks = [make_chunk(
            "doc",
            "doc.pdf",
            "c1",
            "Axone is traditionally prepared by the SÃ¼mi community.",
        )]
        record = {
        ...
        verified = EvidenceVerifier("table1").verify(
            record,
            chunks,
            target_food="axone",
        )

        self.assertEqual(verified["ethnic_group"], "Sümi")
        self.assertEqual(
            verified["evidence_verification"]["fields"]["ethnic_group"],
            "encoding_normalized",
        )


class EntityAndAggregationTests(unittest.TestCase):
    def test_food_validation_rejects_microbes_communities_and_generic_names(self):
        content = (
            "Fermented foods include Axone. Bacillus spp. was isolated from it "
            "and the Barman community prepares traditional products."
        )

        self.assertTrue(is_plausible_food_name("Axone", content))
        self.assertFalse(is_plausible_food_name("Bacillus spp.", content))
        self.assertFalse(is_plausible_food_name("Barman community", content))
        self.assertFalse(is_plausible_food_name("fermented foods", content))

    def test_table7_counts_distinct_papers_not_mentions_or_foods(self):
        records = [
            {
                "taxonomy_name": "Lactobacillus plantarum",
                "document_id": "paper-1",
            ...
            {
                "taxonomy_name": "_Lactobacillus plantarum_",
                "document_id": "paper-1",
                "food_id": "F0002",
            },
            {
            ...
            {
                "taxonomy_name": "Bacillus subtilis",
                "document_id": "paper-2",
                "food_id": "F0001",
            },
        ]

        frequencies = build_microbe_paper_frequency(records)

        self.assertEqual(frequencies, [
            {"microbe": "Lactobacillus plantarum", "count": 2},
            {"microbe": "Bacillus subtilis", "count": 1},
        ])
...
class ExtractionPipelineTests(unittest.TestCase):
    def test_existing_food_ids_are_not_renumbered(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotIn("table7", pipeline.extractors)
    ...
    def test_table_extraction_retries_truncated_json_with_less_context(self):
        from src.extraction.schemas import FoodMasterExtraction
        from src.extraction.table_extractor import StructuredTableExtractor

        llm = InvalidJsonThenSucceedLLM()
        extractor = StructuredTableExtractor(llm, "table1", FoodMasterExtraction)
        chunks = [
            make_chunk("doc", "doc.pdf", f"c{index}", "Kimchi evidence.", index)
            for index in range(1, 4)
