...
class ExtractionPipeline:
    ...
    def _discover(self, json_files: list[Path]):
        ...
        for file_num, file in enumerate(json_files,1):
            ...
            for chunk in chunks:
                ...
                if cached and cached.get("content_hash") == content_hash:
                    foods = self.food_discovery.validate_candidates(
                        cached.get("foods", []),
                        chunk,
                    )
                    cached["foods"] = foods
            ...
            logger.info(f"Files Processeed : {file_num}")
    ...
    def _save_outputs(self, food_records, occurrences, masters, records, failures):
        ...
        self._save_json_atomic(
            build_microbe_paper_frequency(records["table6"]),
            self.tables_dir / "table7.json",
        )
