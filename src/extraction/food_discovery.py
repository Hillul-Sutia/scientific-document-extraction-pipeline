...
class FoodDiscovery:
    ...
    def validate_candidates(self, foods: list[str], chunk: dict) -> list[str]:
        normalized = {
        ...
        validated = sorted(
            ...
            if self._is_explicitly_present(food, chunk)
            and is_plausible_food_name(food, chunk.get("content", ""))
        )
        rejected = sorted(normalized.difference(validated))
        if rejected:
            logger.warning(
                "Food discovery rejected invalid candidates chunk=%s foods=%s",
                chunk.get("chunk_id"),
                rejected,
            )
        return validated
    ...
    def extract_food_names_and_validate(self, chunk: dict) -> list[str]:
        ...
        validated = self.validate_candidates(foods, chunk)
        logger.info(
