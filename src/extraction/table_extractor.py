...
- type describes the product form, for example Solid Fermented Food,
  ...
  Fermented Beverage.
- ethnic_group is a tribe or community explicitly associated with the food in
  the same sentence, table row, or immediately associated heading and paragraph.
""",
    "table2": """
...
Ignore author affiliations, publisher addresses, laboratory addresses, and
locations mentioned only for another food. The food and geographic/community
value must occur in the same sentence, table row, or directly associated heading.
""",
    "table4": """
...
Reject foods, communities, ingredients, generic terms such as "bacteria", and
non-taxonomic microbial groups. The food and taxon must be connected in the same
...
"""
    ...
    def extract(self, food_name: str, chunks: list[dict]) -> list[dict]:
        ...
        while True:
            ...
            try:
                response = self.llm_client.generate(
                    ...
                    max_output_tokens=MAX_OUTPUT_TOKENS[self.table],
                )
            except TimeoutError:
            ...
            try:
                ...
                if len(working_chunks) == 1:
                    raise ValueError(
                        f"Invalid {self.table} response: {exc}"
                    ) from exc
                reduced_count = max(1, len(working_chunks) // 2)
                logger.warning(
                    "Invalid or truncated JSON table=%s food=%s; reducing "
                    "evidence chunks from %s to %s and retrying: %s",
                    self.table,
                    food_name,
                    len(working_chunks),
                    reduced_count,
        ...
        for record in records:
            ...
            data = self.evidence_verifier.verify(
                data,
                working_chunks,
